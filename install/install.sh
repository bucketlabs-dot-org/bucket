...
case "${ARCH}" in
    ...
    i*86|i386) ARCH="386" ;;  #  support 32-bit
...
RELEASE_URL="${URI}/v${VERSION}/bucket-cli-${VERSION}-${OS_SUFFIX}_${ARCH}"
...
if ! curl -sSL "$RELEASE_URL" -o "$TMP_BIN"; then
    error "Failed to download binary from $RELEASE_URL"
fi
